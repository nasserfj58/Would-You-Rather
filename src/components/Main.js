                    ...
                    <Tabs defaultActiveKey="unanswered" id="questions-tab">
                        <Tab eventKey="answered" title="Answered Questions">
                          <ul>
                                {this.props.answerdQuestions.map((question, i) => (
                                     <li style={{display: 'inline-block',margin:'7px', height:'20%', width:'25%'}}><Question question={question} isAnswerd={true} history={this.props.history}/></li>
                            ))}
                        ...
                        <Tab eventKey="unanswered" title="Unanswered Questions">
                            ...
                            {this.props.unanswerdQuestions.map((question, i) => (
                                  <li style={{display: 'inline-block',margin:'7px', height:'20%', width:'25%'}}><Question question={question} isAnswerd={false} history={this.props.history}  /></li>
                            ))}
                            </ul>
                        </Tab>
                    </Tabs>
...
function mapStateToProps({ user, users, questions }) {
    ...
    return {
        answerdQuestions: anwerdQ.map((value) => ({ id: value.id, author: value.author, option1: value.optionOne.text, option2: value.optionTwo.text, time: value.timestamp, answer : value.optionOne.votes.includes(user)?"1":"2", logo:users[value.author].avatarURL, op1P:!value.optionOne.votes.length && !value.optionTwo.votes.length? 0 : (value.optionOne.votes.length /(value.optionOne.votes.length+value.optionTwo.votes.length))*100,op2P:!value.optionOne.votes.length && !value.optionTwo.votes.length? 0 :(value.optionTwo.votes.length /(value.optionOne.votes.length+value.optionTwo.votes.length))*100,op1AnswerdBy:value.optionOne.votes.length,op2AnswerdBy:value.optionTwo.votes.length })),
